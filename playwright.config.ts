...
export default defineConfig({
  ...
  projects: [
    // Testes sem autenticação (landing, auth forms, UX validação)
    {
      ...
      testIgnore: ['**/library.spec.ts', '**/review.spec.ts', '**/accessibility.spec.ts', '**/crud-01.spec.ts', '**/sprint1.spec.ts'],
    },
    ...
    // Testes autenticados (library, review, accessibility, crud-01) — requerem user.json gerado pelo globalSetup
    {
      ...
      testMatch: ['**/library.spec.ts', '**/review.spec.ts', '**/accessibility.spec.ts', '**/crud-01.spec.ts', '**/sprint1.spec.ts'],
    },
  ],
