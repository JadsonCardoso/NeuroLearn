...
function LoginForm() {
  ...
  // Fecha o tooltip ao clicar fora ou pressionar ESC
  useEffect(() => {
    if (!showGoogleTooltip) return
    function handleOutside(e: MouseEvent) {
      if (googleBtnRef.current && !googleBtnRef.current.contains(e.target as Node)) {
        setShowGoogleTooltip(false)
      }
    }
    function handleEsc(e: KeyboardEvent) {
      if (e.key === 'Escape') setShowGoogleTooltip(false)
    }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div className="slide-in" style={{ width: '100%', maxWidth: '400px' }}>
          ...
          {/* Botão Google OAuth — disponível em breve */}
          <div ref={googleBtnRef} style={{ position: 'relative' }}>
            <button
              data-testid="btn-google-oauth"
              type="button"
              disabled
              onClick={() => setShowGoogleTooltip((v) => !v)}
              style={{
                width: '100%',
                display: 'flex',
                alignItems: 'center',
            ...
            {showGoogleTooltip && (
              ...
              >
                🚧 Em breve — estamos preparando o login com Google
              </div>
            )}
          </div>
