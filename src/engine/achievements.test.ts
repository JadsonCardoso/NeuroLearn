...
const cards = (n: number) => Array.from({ length: n }, (_, i) => ({ id: `card-${i}` })) as AppState['cards']
const sessions = (n: number) => Array.from({ length: n }, (_, i) => ({ id: `session-${i}` })) as AppState['sessions']
...
describe('streak_7', () => {
  it('bloqueado com streak = 6', () => {
    expect(computeAchievements(makeState({ streak: 6 })).find((a) => a.id === 'streak_7')?.unlocked).toBe(false)
  })

  it('desbloqueado com streak = 7', () => {
    expect(computeAchievements(makeState({ streak: 7 })).find((a) => a.id === 'streak_7')?.unlocked).toBe(true)
  })
})

describe('streak_30', () => {
  it('bloqueado com streak = 29', () => {
    expect(computeAchievements(makeState({ streak: 29 })).find((a) => a.id === 'streak_30')?.unlocked).toBe(false)
  })

  it('desbloqueado com streak = 30', () => {
    expect(computeAchievements(makeState({ streak: 30 })).find((a) => a.id === 'streak_30')?.unlocked).toBe(true)
  })
})

describe('cards_10', () => {
  it('bloqueado com 9 cards', () => {
    expect(computeAchievements(makeState({ cards: cards(9) })).find((a) => a.id === 'cards_10')?.unlocked).toBe(false)
  })

  it('desbloqueado com 10 cards', () => {
    expect(computeAchievements(makeState({ cards: cards(10) })).find((a) => a.id === 'cards_10')?.unlocked).toBe(true)
  })
})

describe('cards_50', () => {
  it('bloqueado com 49 cards', () => {
    expect(computeAchievements(makeState({ cards: cards(49) })).find((a) => a.id === 'cards_50')?.unlocked).toBe(false)
  })

  it('desbloqueado com 50 cards', () => {
    expect(computeAchievements(makeState({ cards: cards(50) })).find((a) => a.id === 'cards_50')?.unlocked).toBe(true)
  })
})

describe('cards_100', () => {
  it('bloqueado com 99 cards', () => {
    expect(computeAchievements(makeState({ cards: cards(99) })).find((a) => a.id === 'cards_100')?.unlocked).toBe(false)
  })

  it('desbloqueado com 100 cards', () => {
    expect(computeAchievements(makeState({ cards: cards(100) })).find((a) => a.id === 'cards_100')?.unlocked).toBe(true)
  })
})

describe('reviews_10', () => {
  it('bloqueado com 9 sessões', () => {
    expect(computeAchievements(makeState({ sessions: sessions(9) })).find((a) => a.id === 'reviews_10')?.unlocked).toBe(false)
  })

  it('desbloqueado com 10 sessões', () => {
    expect(computeAchievements(makeState({ sessions: sessions(10) })).find((a) => a.id === 'reviews_10')?.unlocked).toBe(true)
  })
})

describe('reviews_50', () => {
  it('bloqueado com 49 sessões', () => {
    expect(computeAchievements(makeState({ sessions: sessions(49) })).find((a) => a.id === 'reviews_50')?.unlocked).toBe(false)
  })

  it('desbloqueado com 50 sessões', () => {
    expect(computeAchievements(makeState({ sessions: sessions(50) })).find((a) => a.id === 'reviews_50')?.unlocked).toBe(true)
  })
})

describe('xp_100', () => {
  it('bloqueado com 99 XP', () => {
    expect(computeAchievements(makeState({ totalXp: 99 })).find((a) => a.id === 'xp_100')?.unlocked).toBe(false)
  })

  it('desbloqueado com 100 XP', () => {
    expect(computeAchievements(makeState({ totalXp: 100 })).find((a) => a.id === 'xp_100')?.unlocked).toBe(true)
  })
...
describe('xp_1000', () => {
  it('bloqueado com 999 XP', () => {
    expect(computeAchievements(makeState({ totalXp: 999 })).find((a) => a.id === 'xp_1000')?.unlocked).toBe(false)
  })

  it('desbloqueado com 1000 XP', () => {
    expect(computeAchievements(makeState({ totalXp: 1000 })).find((a) => a.id === 'xp_1000')?.unlocked).toBe(true)
  })
})
